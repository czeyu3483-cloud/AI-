...
> **Demo 收敛**：页面展示多岗位与多风格，但 **仅「研发岗 + 压力面」可选**；其余选项保留并标注「（暂不可选）」——详见 [`DEMO_ROLE_AND_STYLE.md`](./DEMO_ROLE_AND_STYLE.md)。

---

## 1. 产品一句话与成功标准

**产品**：浏览器内可用的仿真面试官——Demo 开放研发岗压力面；支持简历输入、数字人视频 + 双向实时语音，按题库与可配置状态机完成「出题 → 作答 → 有限追问/引导 → 复盘反馈 + PDF 报告」。

**MVP 成功标准（缺一不可）**

1. 首页展示多岗位/多风格占位；用户仅能选 **研发岗 + 压力面**，并输入简历（粘贴文本 / 上传 txt、docx、pdf）后开面（无需登录）
2. 面试官以视频数字人 + TTS 语音提问；至少部分题目锚定简历项目；压力面参数真实作用于留白/追问/提示
3. 用户麦克风作答（ASR → 文本）
4. 状态机按规则处理：正常答 / 思考中 / 卡壳，并决定追问、温和提示或下一题
5. 结束后输出结构化复盘，并可下载 PDF 报告（含本场岗位与风格）；全程有基础日志
...
风格预设：Demo 运行只合并 `config/styles/pressure.json`。`calm.json` 可留在仓库供二期，但 UI 上平和/随机为「（暂不可选）」，`start` 仅接受 `pressure`（见 [`DEMO_ROLE_AND_STYLE.md`](./DEMO_ROLE_AND_STYLE.md)）。
...
| 方法 | 路径 | 作用 |
| --- | --- | --- |
| `GET` | `/api/roles` | 返回岗位列表（含 `enabled`；仅研发岗 true） |
| `POST` | `/api/resume/parse` | 粘贴文本或上传 txt/docx/pdf → `ResumeProfile` |
| `POST` | `/api/interview/start` | body 仅接受 `roleId=rd_general` + `style=pressure` + 可选简历 |
...
1. **首页**：岗位列表（研发岗可选，其它「暂不可选」）+ 风格列表（**仅压力面可选**，平和/随机「暂不可选」）
...
3. **面试页（主界面）**
   ...
   - 控制：静音、结束面试（无登录）
4. **复盘页**：结构化反馈（含岗位/风格）；**下载 PDF 报告**；可「再面一场」
5. **（可选）调试页**：行为参数滑杆

不做：隐藏其它岗位/风格、仪表盘、历史列表、账号墙、复杂设置中心、ATS 级简历诊断站。
...
2. 写入 **研发岗** 题库 JSON + 评分维度；`config/styles/pressure.json`（calm 预设可先放仓库但不启用）
3. 实现 Rule Engine 纯函数 + 单元测试（状态迁移、追问计数、停顿阈值、**控场动作与红线词**、**压力面参数**）
4. 固化话术模板库：`TIMEBOX` / `REDIRECT` / `SKIP_SOFT` / `FORMULA_DEFLECT` 等（压力面 `steady_firm` 措辞）
5. **简历解析**：paste/txt/docx/pdf → `ResumeProfile` 预览编辑；`start` 校验仅 rd+pressure 并组卷
6. `start / turn / silence / finish` API 打通（TTS/数字人先返回纯文本）
7. 反馈强制输出 AbilityTag + 本场「研发岗 · 压力面」
8. 首页选项数据：`enabled` 字段驱动「（暂不可选）」态

**阶段出口**：纯文字模式下完整跑通「页面可见多岗位/多风格占位 → 仅研发+压力可开面 → 简历 → 一次一问 → 结构化标签反馈 + 日志」
...
- 表情/眼神/情绪
- 多人面试、可视化大数据看板、付费
- 解开其它岗位/风格的 `enabled`（题库与 calm 参数就绪后再做）
...
- [ ] 页面展示多岗位；仅研发岗可选；其它带「（暂不可选）」且不可开面
- [ ] 页面展示压力面 / 平和面（暂不可选）/ 随机（暂不可选）；仅压力面可提交
- [ ] 服务端拒绝非研发岗或非压力面的 start
- [ ] 研发岗 + 压力面 → 简历（文本/txt/docx/pdf）→ 开面，无登录
- [ ] 解析可预览编辑；失败可回退粘贴；提示脱敏
- [ ] 至少 1 题锚定简历项目；追问能点名项目/技术栈
- [ ] 压力面：较短沉默阈值、少提示、边界/细节追问可感知
- [ ] 数字人视频态 + TTS 提问可感知
- [ ] 麦克风作答进入流程
...
见 [`DEMO_ROLE_AND_STYLE.md`](./DEMO_ROLE_AND_STYLE.md)。运行时固定：
...
**方案结论**：以 **「知识库 + 同构面试状态机」为中枢**；UI 预留多岗位/多风格但 Demo **仅解锁研发岗 + 压力面**；简历解析与 PDF 报告为闭环两端。开发顺序：**文字闭环（压力面+简历组卷）→ 语音 → 数字人 → PDF 报告与交付**。
