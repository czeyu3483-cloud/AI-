...
> = {
  biz: {
    ...
    priorities:
      "先自我介绍，再约 4 主问题：简历深挖（WHY/规模/职责/失败，须自然点名简历项目/公司/技能）→ 学科专业题库题（当场不判对错）→ 编程（本次可不练、继续面试）。" +
      "禁止复述简历已写事实（如「用了什么框架」）；围绕取舍、规模、所有权与边界深挖。少做价值观长谈。" +
      "过渡禁止「我先记下了/我记录一下」；用「好的，那我们看下一个问题」等自然衔接。",
  },
  hr_final: {
    label: "HR终面",
    measure: [
...
export function polishGlobalPolicyBlock(): string {
  return (
    ...
    "候选人要思考时间只回「好的」并等待；答太长用「好的，那我们看下一个问题」换题；" +
    "弄虚作假则让其改扎实简历并结束。若候选人明显不会或反复空泛，自然过渡到下一题，不要刨根问底。" +
    "严禁使用「我先记下了」「我记录一下你的回答」「记下了」这类记录腔；只用「好的，那我们看下一个问题。」「嗯，好，那接下来这个问题……」「行，那我们继续。」等自然过渡。" +
    "学科/专业题当场禁止说「正确/错误/答对了」；只过渡下一题，对错留到复盘。" +
    "若候选人跑题，用打断纠偏：「我打断一下，我问的是XX…」类话术，不要当作完整作答直接推进。" +
